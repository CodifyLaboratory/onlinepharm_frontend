...
function Ordering({ navigation, route }) {
    ...
    const submit = async (data) => {
        let form = {
            ...
            order_medications: basket.map((item) => ({
    ...
    return (
        <ScrollView style={{ backgroundColor: '#fff' }}>
            <View style={ordering.container}>
                <Text style={ordering.title}>
                    {strings.cart.contact_details}
                </Text>
                ...
                <Controller
                    ...
                    name="phone"
                />
                <Text style={ordering.label}>
                    {strings.cart.order_comments}
                </Text>
                <Controller
                    control={control}
                    rules={{ required: false }}
                ...
                <Text style={{ ...ordering.label, marginTop: 20 }}>
                    {strings.cart.city}:
                </Text>
                <Controller
                    control={control}
                ...
                <TouchableOpacity
                    ...
                    style={ordering.mapSelectBtn}
                >
                    <Text style={ordering.mapSelectBtnText}>
                        {strings.cart.choose_on_map}
                    </Text>
                </TouchableOpacity>

                <Text style={ordering.productListTitle}>
                ...
                <View style={[ordering.boxes, { marginBottom: 16 }]}>
                    <Text style={ordering.text}>
                        {strings.cart.shipping_cost}
                    </Text>
                ...
                <View style={[ordering.boxes]}>
                    ...
                    <Text style={ordering.text}>
                        {total + delivery_price} с
                    </Text>
                </View>

                <Text style={ordering.productListTitle}>
                    {strings.cart.payment_methods}
                </Text>
                ...
                <NativeBaseProvider>
                    ...
                    >
                        <Radio style={ordering.radioBtn} value={1} my={1}>
                            <Text style={ordering.radioText}>
                                {strings.cart.in_cash}
                            </Text>
                        </Radio>
                        {/*<Radio*/}
                        {/*    style={ordering.radioBtn}*/}
                ...
                {paymentMethodsValue === 2 && (
                    <View style={{ marginBottom: 34, marginTop: 30 }}>
                        {cards.map(({ type, number }) => (
                            <View style={ordering.paymentCardWrap}>
                                <View style={ordering.paymentIcon}>
                                    <VisaIcon />
                                </View>
                                <View>
                                    <Text style={ordering.cardName}>
                                        {type}
                                    </Text>
                                    <Text style={ordering.cardNumber}>
                                        {number}
                                    </Text>
                                </View>
                            </View>
                ...
                <TouchableOpacity
                    onPress={handleSubmit(submit)}
                    style={{ ...cartStyle.btn, marginTop: 20 }}
                    activeOpacity={0.8}
                >
                    <Text style={cartStyle.btnText}>
            ...
            <SuccessModalComponent
                text={strings.cart.successfully_placed}
                visible={showModal}
                setVisible={(e) => setShowModal(e)}
            />
            <MapModal
                setValue={(e) => setValue('address', e)}
                visible={showMap}
                setShow={(e) => setShowMap(e)}
            />
        </ScrollView>
    )
}
